...
$\forall \e >0, \exist N \in N_+$, 当 $n>N$ 时，对任意 $p \in N_+$ ，有 $$\v u_{n+1}+u_{n+2}+\dots u_{n+p} \v < \e$$ 
...
2. **比较审敛法**
	...
	+ 若**强级数** $\infsum v_n$ 收敛, 则 **弱级数** $$\infsum u_n$$ 也收敛
	+ 若 **弱级数**  发散, 则 **强级数** 也发散
3. **比较审敛法的极限形式**
	正项级数 $$\infsum u_n, \infsum v_n$$ 满足 $$\lim_{n\to\infty} \frac{u_n}{v_n} = l$$
	+ $0 < l <\infty$, 两个级数同时收敛或发散
	+ $l = 0$ 且 $$\infsum v_n$$ 收敛时， $$\infsum u_n$$ 也收敛
	+ $l = \infty$ 且 $$\infsum v_n$$ 发散时， $$\infsum u_n$$ 也发散
...
形如 $$\infsum a_n (x - x_0)^n = a_0 + a_1(x-x_0) + a_2(x - x_0)^2 + \cdots + a_n(x - x_n)^n + \cdots$$
...
+ $R = 0$ 时，幂级数仅在 $x= 0$ 收敛
+ $R = +\infty$ 时， 幂级数在 $(-\infty, +\infty)$ 收敛
+ $0<R<+\infty$, 幂级数在 $(-R,R)$ 收敛， $[-R,R]$ 外发散，在 $x=\pm R$ 可能收敛也可能发散
...
若 $$\infsum a_n x^n$$ 的系数满足 $$\lim_{n\to \infty} \v \dfrac{a_{n+1}}{a_n} \v = \rho$$, 则
...
3. $$
	\frac 1{1+x} = \infsum (-1)^nx^n, ~~~~~x \in (-1,1)
	$$
...
$$
\begin{cases}
a_n = \frac 1\pi \int_{-\pi}^{\pi} f(x) \cos nx ds &(n=0,1,\dots)\\
